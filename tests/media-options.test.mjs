...
function still(sandbox) {
  const file = path.join(sandbox.workspace, "still.png");
...
test("animate asks for 720p and 6 s by default, with no aspect ratio", async (t) => {
  ...
  await generate(sandbox, ["animate", "the kite drifts", "--image", still(sandbox)], [{ tool: "image_to_video" }]);
...
test("--draft keeps an explicit --duration", async (t) => {
  ...
  await generate(sandbox, ["animate", "drift", "--image", still(sandbox), "--draft", "--duration", "10"], [{ tool: "image_to_video" }]);
...
test("--aspect on animate is refused, explaining the video keeps the image's shape", async (t) => {
  ...
  await assertRejectedBeforeGrok(
    sandbox,
    ["animate", "drift", "--image", still(sandbox), "--aspect", "16:9"],
...
test("edit --aspect with a single image is refused: only multi-image edits take one", async (t) => {
  ...
  await assertRejectedBeforeGrok(
    sandbox,
    ["edit", "make it blue", "--image", still(sandbox), "--aspect", "16:9"],
